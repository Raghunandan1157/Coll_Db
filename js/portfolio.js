/**
 * portfolio.js — Portfolio tab renderer for the employee dashboard.
 * Renders month-end report data with sheet toggle, on-date preview,
...
(function () {
  ...
  /* ========== Fuzzy name matching (same as employee.js) ========== */
  ...
  /* ===================== RENDERING ===================== */
  var _pfState = { view: 'overall', product: 'all', rows: null, fyRows: null,
                   onDateRows: null, tomRows: null, fyOnDateRows: null, fyTomRows: null,
                   bounds: null, fyBounds: null, sheetMap: null };

  function getActiveRows() { return _pfState.view === 'fy' ? _pfState.fyRows : _pfState.rows; }
  function getActiveBounds() { return _pfState.view === 'fy' ? _pfState.fyBounds : _pfState.bounds; }
  function getActiveOnDate() { return _pfState.view === 'fy' ? _pfState.fyOnDateRows : _pfState.onDateRows; }
  function getActiveTom() { return _pfState.view === 'fy' ? _pfState.fyTomRows : _pfState.tomRows; }
  ...
  function renderPortfolio() {
    ...
    var empRow;
    if (_pfState.product === 'all') {
      empRow = findEmpRow(rows);
    } else {
      empRow = findProductRow(rows, getActiveBounds(), _pfState.product);
    }
    ...
    if (session.role && session.role !== 'FO') {
      ...
      if (childRole) {
        ...
        if (_pfState.product === 'all') {
          ...
          children = findProductChildren(rows, getActiveBounds(), _pfState.product);
          children.sort(function (a, b) {
  ...
  function viewToggleHtml() {
    var ov = _pfState.view === 'overall';
    return '<div class="pf-view-toggle emp-fade">' +
      '<button class="pf-view-btn' + (ov ? ' active' : '') + '" data-view="overall">OverAll</button>' +
      '<button class="pf-view-btn' + (!ov ? ' active' : '') + '" data-view="fy">FY 25-26</button>' +
    '</div>';
  }
  ...
  function productPillsHtml() {
    var p = _pfState.product;
    return '<div class="emp-product-filter">' +
      '<button class="emp-product-pill' + (p === 'all' ? ' active' : '') + '" data-pf-product="all">All</button>' +
      '<button class="emp-product-pill' + (p === 'igl' ? ' active' : '') + '" data-pf-product="igl">IGL</button>' +
      '<button class="emp-product-pill' + (p === 'fig' ? ' active' : '') + '" data-pf-product="fig">FIG</button>' +
      '<button class="emp-product-pill' + (p === 'il' ? ' active' : '') + '" data-pf-product="il">IL</button>' +
    '</div>';
  }
  ...
  function onDateSectionHtml(onDateRows, tomRows) {
    ...
    // On-date columns: B=1 name, C=2 demand, D=3 collection, E=4 pct
  ...
  function attachHandlers() {
    ...
    // View toggle
    container.querySelectorAll('.pf-view-btn').forEach(function (btn) {
      btn.onclick = function () {
        _pfState.view = btn.dataset.view;
        renderPortfolio();
      };
    });
    ...
    container.querySelectorAll('[data-pf-product]').forEach(function (pill) {
      pill.onclick = function () {
        _pfState.product = pill.dataset.pfProduct;
        renderPortfolio();
      };
    });
    ...
    if (!_pfHandlersAttached) {
      ...
      container.addEventListener('click', function (ev) {
        ...
        if (!card) return;
        // Only handle if we're in the portfolio tab
  ...
  window._loadPortfolioTab = async function () {
    try {
      ...
      _pfState.sheetMap = sheetMap;

      // Parse all sheets
      _pfState.rows = parseRows(workbook, sheetMap.overall);
      _pfState.fyRows = parseRows(workbook, sheetMap.fy);
      _pfState.onDateRows = parseRows(workbook, sheetMap.overallOnDate);
      _pfState.tomRows = parseRows(workbook, sheetMap.overallTom);
      _pfState.fyOnDateRows = parseRows(workbook, sheetMap.fyOnDate);
      _pfState.fyTomRows = parseRows(workbook, sheetMap.fyTom);

      // Detect product boundaries
      if (_pfState.rows) _pfState.bounds = detectProductBounds(_pfState.rows);
      if (_pfState.fyRows) _pfState.fyBounds = detectProductBounds(_pfState.fyRows);
