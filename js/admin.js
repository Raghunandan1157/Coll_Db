...
(function () {
  ...
  async function pushToServer(arrayBuffer, category) {
    ...
    if (!res.ok) {
      ...
      throw new Error('Upload failed (' + res.status + '): ' + (err.error || err.message || 'Unknown error'));
    }
  ...
  async function handleCategoryFile(category, file, card) {
    ...
    try {
      ...
      if (category === 'collection') {
        ...
        try {
          await pushToServer(arrayBuffer, 'collection');
        ...
        showCardStatus(card, 'Syncing to cloud...', true);
        try {
          await pushToServer(arrayBuffer, category);
          showCardStatus(card, 'Uploaded & synced!', true);
        } catch (ghErr) {
          console.warn('GitHub sync failed:', ghErr);
          showCardStatus(card, 'Saved locally. Sync failed.', false);
