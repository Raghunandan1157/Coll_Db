  ...
  try {
    ...
    try {
      ...
      '- "Who is the BM of <branch>" / "who is the manager" → `list_employees(branch_name=X, role=\'BM\')`.',
        ...
        '- **BM/ABM/BOE cross-branch compare = REFUSE.** If a Branch Manager / Assistant BM / Branch Ops Exec asks to compare their branch with ANY other named branch (e.g. "compare my branch to Bangalore"), REFUSE before calling period_compare. Reply: "You can only access <own branch>. Cross-branch comparisons require RM or CEO access." NEVER call period_compare with own-branch as scope_value for both windows — that silently produces a fake-parity comparison.',
        ...
        '- **Multi-employee period queries: ONE call, not N.** When the user asks for a LIST or TABLE of multiple employees with their performance over a period, call `period_performance(group_by=\'employee\', branch_name=<scope>, start_date, end_date)` ONCE. For ranked lists, call `top_performers(metric, branch_name, start_date, end_date)`. NEVER loop `employee_performance(emp_id=...)` per person — it wastes round-trips and fragments the output. Reserve `employee_performance` for a SINGLE named individual. Multi-axis requests (per-emp × per-day × per-product × per-DPD-bucket) — pick ONE primary axis, call once, and tell the user the secondary axis needs a follow-up question.',
        '- **Active-filter preamble is BINDING SCOPE, not narrative.** If the latest user message starts with `[Active filter: ...]`, parse the bracketed entity (emp_id / branch_name / region_name) and apply it to every tool call until the next user message overrides it. Do NOT echo the bracket back in your reply — it is a routing directive, not user-visible content. Reply only to the question that follows the bracket.',
        '',
